...
description: 在非生产环境启用开发工作台、集成替身、本地捕获和测试身份。
edition: ce
audience:
  - plugin-developer
  - operator
...
本文只说明 CE 提供的开发模式能力；其他版本增加的产品行为不在本文范围内。
...
- `plugin_signature: allow_unsigned` 不会提供插件包密码学签名，也不会关闭
  SHA-256、ZIP 路径或 manifest 校验。只安装已经人工审查的本地插件包。
- 开发模式状态通过 health、后台任务页、Workbench、响应头和持久审计查看；当前没有
  独立的 Prometheus 指标端点。
- `bin/dev` 会同时启动 Rails、Tailwind watcher 与 Vite dev server/HMR；直接运行
  `bin/rails server` 不会管理这些额外进程。
- 开发模式只适用于隔离的本地环境。不要把在本机通过的行为视为反向代理、HTTPS、
  邮件、支付、Webhook 或 Web Push 的生产验收结果。
