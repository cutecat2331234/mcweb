import type { ApplicationShellNavigationGroup } from '../app/javascript/lib/applicationShell.ts'
import type {
  FrontendApplicationBudget,
  FrontendApplicationContribution,
  FrontendApplicationDescriptor,
  FrontendApplicationLauncher,
  FrontendApplicationRegistryData,
  FrontendComponentClaim as ComponentClaim,
  FrontendDraftContract,
  FrontendProjection,
  FrontendRendererContract,
  FrontendRouteKind,
  FrontendRouteRule,
  FrontendRuntimeKind,
} from '../app/javascript/lib/frontendApplicationTypes.ts'
import {
  createFrontendApplicationRegistry,
  routePatternExpression,
} from '../app/javascript/lib/frontendApplicationRegistry.ts'

// The single JavaScript manifest compiler, shared by repository checks and
// Vite. None of this validation code may be imported into browser chunks.
export type FrontendApplicationManifestSources = Readonly<{
  baseManifestModules: Readonly<Record<string, unknown>>
  contributionManifestModules: Readonly<Record<string, unknown>>
  sharedRoutesManifest: unknown
}>
...
export function compileFrontendApplicationRegistry({
  baseManifestModules,
  contributionManifestModules,
  sharedRoutesManifest,
}: FrontendApplicationManifestSources): FrontendApplicationRegistryData {
  ...
  const data: FrontendApplicationRegistryData = {
    applications: [...applications.values()].map(freezeApplication)
      .sort((left, right) => left.id.localeCompare(right.id)),
    routeRules: [...routeRules],
    componentClaims: [...componentClaims],
    launchers: launchers.map(({ path, applicationId }) => ({ path, applicationId })),
  }
  const {
    frontendApplications,
    resolveFrontendRoute,
    assertFrontendComponent,
  } = createFrontendApplicationRegistry(data)
