...
const groupedBadges = computed(() => {
  ...
  for (const badge of props.badges) {
    const key = badge.grouping || 'general'
    if (!groups.has(key)) groups.set(key, [])
    groups.get(key)!.push(badge)
  }
  return Array.from(groups, ([grouping, items]) => ({ grouping, items }))
})
</script>
...
<template>
  ...
  <div v-if="badges.length" class="space-y-8">
    <section v-for="group in groupedBadges" :key="group.grouping">
      <h2 class="mb-3 text-sm font-semibold uppercase tracking-wide text-muted-foreground">{{ group.grouping }}</h2>
        ...
        >
          <div class="flex items-start gap-3">
            ...
            <div class="min-w-0 flex-1">
              <h3 class="font-semibold" :style="{ color: badge.color }">{{ badge.name }}</h3>
