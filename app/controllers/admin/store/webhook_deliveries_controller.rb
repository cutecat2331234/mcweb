  ...
  module Store
    class WebhookDeliveriesController < Admin::BaseController
      ...
      def index
        ...
        render inertia: "Admin/Generic/Index", props: {
          title: t("mcweb.admin.store.webhook_deliveries.title"),
          ...
          columns: [
            admin_column(:order, t("mcweb.admin.store.webhook_deliveries.col_order"), link: true),
            admin_column(:event, t("mcweb.admin.store.webhook_deliveries.col_event")),
      ...
      def show
        render inertia: "Admin/Generic/Show", props: {
          title: t("mcweb.admin.store.webhook_deliveries.show_title", id: @delivery.id),
          subtitle: @delivery.order_public_id,
          fields: [
            { label: t("mcweb.admin.store.webhook_deliveries.field_order"), value: @delivery.order_public_id.presence || t("mcweb.labels.not_available") },
            { label: t("mcweb.admin.store.webhook_deliveries.field_status"), value: webhook_delivery_status_label(@delivery.status) },
            { label: t("mcweb.admin.store.webhook_deliveries.field_event"), value: @delivery.event_type },
            { label: t("mcweb.admin.store.webhook_deliveries.field_response_code"), value: @delivery.response_code&.to_s || t("mcweb.labels.not_available") },
            { label: t("mcweb.admin.store.webhook_deliveries.field_attempt_count"), value: @delivery.attempt_count.to_s },
            { label: t("mcweb.admin.store.webhook_deliveries.field_url"), value: @delivery.url },
            { label: t("mcweb.admin.store.webhook_deliveries.field_time"), value: l(@delivery.created_at, format: :short) }
          ],
    ...
    private
      ...
      def webhook_status_tabs
        ...
        tabs = [ { label: t("mcweb.admin.store.filter_all"), href: base, active: current.blank?, count: total } ]
        {
          "failed" => t("mcweb.admin.store.webhook_deliveries.status_failed"),
          "success" => t("mcweb.admin.store.webhook_deliveries.status_success"),
          "pending" => t("mcweb.admin.store.webhook_deliveries.status_pending")
      ...
      def webhook_event_tabs
        ...
        tabs = [ { label: t("mcweb.admin.store.filter_all_events"), href: base, active: current.blank?, count: total } ]
      ...
      def webhook_kind_tabs
        ...
        [
          { label: t("mcweb.admin.store.filter_all_sources"), href: base, active: current.blank? },
          { label: t("mcweb.admin.store.webhook_deliveries.kind_test"), href: admin_store_webhook_deliveries_path(base_params.merge(kind: "test")), active: current == "test" },
          { label: t("mcweb.admin.store.webhook_deliveries.kind_production"), href: admin_store_webhook_deliveries_path(base_params.merge(kind: "production")), active: current == "production" }
        ]
      end

      def serialize_index_row(delivery)
        admin_row(
          order: delivery.order_public_id.presence || t("mcweb.labels.not_available"),
      ...
      def preformatted_sections
        ...
        if @delivery.request_payload.present?
          sections << {
            title: t("mcweb.admin.store.webhook_deliveries.section_request_body"),
            content: JSON.pretty_generate(@delivery.request_payload)
          }
        end
        if @delivery.response_body.present?
          sections << {
            title: t("mcweb.admin.store.webhook_deliveries.section_response_body"),
      ...
      def show_actions
        ...
        [
          {
            label: t("mcweb.admin.store.webhook_deliveries.action_retry"),
      ...
      def bulk_retry_props(deliveries)
        ...
        {
          label: t("mcweb.admin.store.webhook_deliveries.bulk_retry_label", count: failed_ids.size),
