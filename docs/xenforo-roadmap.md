...
- [x] **主题版主私语** — ✅ 复核发现已完整(`Topics/Show.vue` 版主笔记面板 + 创建表单 + `staff_note` action + 序列化);仅缺单条删除(极小,可选)。缺口分析曾误标。
...
- [x] **持久通知 + Web Push / 邮件** — ✅ CE 已完成。WebSocket / ActionCable 实时能力属于 EE 付费版专属,不在 CE 中提供。
...
- [x] **Phrases 运行时 i18n**(DB 覆盖层 + I18n 后端,大件)、**论坛页面节点 CMS**(与帮助中心重叠,可在其上加层级/导航) — ✅ 两项均已完成,详见下方条目。
- [x] **CE 通知交付边界** — ✅ CE 持久化站内通知并通过普通页面/API 轮询或刷新读取,同时保留 Web Push 与邮件交付。CE 不挂载 `/cable`,不包含业务频道、WebSocket 客户端或实时广播；论坛与频道实时同步是 EE 付费版专属能力。
- [ ] **打字指示器** — CE 不提供 WebSocket 打字状态；EE 付费版可通过其专属实时频道提供。
...
> **CE 路线图主体已完成。** 前端 phrases 合并、移动重定向桩、主组切换与提醒查看自动消除均保留；实时私信和打字状态归入 EE 付费版。CE 私信继续采用持久化提交、页面读取与手动刷新。
