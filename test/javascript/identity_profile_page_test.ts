...
test('profile self-service uses Arco fields and synchronizes the shared locale preference', () => {
  ...
  assert.match(page, /headers: localeRequestHeaders\(selectedLocale\)/)
  assert.match(page, /writeSharedAppLocale\(selectedLocale\)/)
  assert.match(page, /writeSharedAppLocale\(previousLocale\)/)
  assert.match(page, /nextPage\.props\.form_errors/)
  assert.match(page, /persistedLocale !== selectedLocale/)
