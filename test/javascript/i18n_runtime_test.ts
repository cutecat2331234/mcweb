...
test('language switchers commit shared locale state only after the cross-application action succeeds', () => {
  ...
  for (const relativePath of localeSwitcherSources) {
    ...
    const removeStaleQuery = source.indexOf("destination.searchParams.delete('locale')")
    const documentVisit = source.indexOf('navigateFrontendDocument(destination)')
    ...
    assert.ok(removeStaleQuery > commit, `${relativePath} must remove the stale locale query after committing`)
    assert.ok(documentVisit > removeStaleQuery, `${relativePath} must remove the stale locale query before navigating`)
