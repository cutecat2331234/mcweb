...
test('admin layout exposes only effective modules and gives identity groups their own section', () => {
  ...
  assert.match(source, /can_access_admin\?: boolean/)
  ...
  assert.match(source, /requiresAdminAccess\?: boolean/)
  assert.match(source, /canAccessAdminDashboard/)
  assert.match(source, /const adminHomeHref = computed/)
  assert.match(source, /return activeItemHref\.value \|\| currentPath\.value/)
  assert.match(source, /<Link :href="adminHomeHref" class="arco-admin-brand__link">/)
  assert.match(source, /<Link :href="adminHomeHref">\{\{ t\('common\.adminPanel'\) \}\}<\/Link>/)
  assert.doesNotMatch(source, /<Link :href="adminRoutes\.dashboard" class="arco-admin-brand__link"/)
...
test('admin entry resolves Arco components and styles on demand', () => {
  ...
  assert.match(source, /createMcWebInertiaApplication/)
  assert.match(source, /providerComponent: false/)
  assert.match(source, /uiAdapterId: 'mcweb_ui'/)
  assert.doesNotMatch(source, /AppProvider|h\(AppProvider/)
  const bootstrap = javascriptSource('lib/createInertiaApplication.ts')
  assert.doesNotMatch(bootstrap, /import AppProvider/)
  assert.match(bootstrap, /providerComponent\?: Component \| false/)
