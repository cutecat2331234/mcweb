---
title: 大应用与插件扩展
description: 识别内核、大应用、可信 Ruby 插件与外部扩展的边界和管理入口。
edition: ce
audience:
  - administrator
  - plugin-developer
...
[Plugin SDK](/docs/plugin-development/sdk/)。
...
McWeb 采用单仓库部署。可信 Ruby 插件不能卸载或替换内建大应用，也不提供远程插件
目录、远程下载、自动升级、发布者签名或不可信代码沙箱。多进程部署安装或切换插件后，
应按发布流程协调重启或重载所有 Rails 与 Worker 进程。
