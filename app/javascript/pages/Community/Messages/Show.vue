...
async function removeParticipant(participant: { username: string; remove_url?: string | null; is_self?: boolean }) {
  ...
  const msg = participant.is_self ? t('forum.messages.confirmLeaveGroup') : t('forum.messages.confirmRemoveMember', { username: participant.username })
  const ok = await confirm({
    title: participant.is_self ? t('forum.messages.leaveGroup') : t('forum.messages.removeMember'),
    message: msg,
    confirmLabel: participant.is_self ? t('forum.messages.leave') : t('forum.messages.removeMember'),
    variant: 'destructive',
  })
  if (!ok) return
...
<template>
  ...
  <div v-if="conversation.is_group && participants.length" class="mb-4">
    ...
    <form v-if="addParticipantUrl && canAddParticipant" class="mt-3 flex max-w-sm gap-2" @submit.prevent="addParticipant">
      <Input v-model="addUsername" :placeholder="t('forum.messages.usernamePlaceholder')" class="flex-1" />
      <Button type="submit" variant="outline" size="sm">{{ t('forum.messages.addMember') }}</Button>
    </form>
