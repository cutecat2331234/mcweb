...
<template>
  <div>
    <button
      type="button"
      :class="cn(
        'mb-1 flex w-full items-center gap-2 rounded-lg px-3 py-1.5 text-left text-xs font-semibold uppercase tracking-wider transition-colors duration-150',
        'hover:bg-sidebar-accent/40',
        hasActiveChild ? 'bg-sidebar-accent/60 text-sidebar-foreground' : 'text-sidebar-foreground/55',
      )"
      @click="emit('toggle')"
    >
      <ChevronRight
