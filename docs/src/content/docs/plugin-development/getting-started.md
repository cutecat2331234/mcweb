---
title: 插件开发入门
description: 从创建插件到运行测试、校验包并准备发布的标准工作流。
edition: ce
audience:
  - plugin-developer
---
...
[Plugin SDK 的商业事件目录](/docs/plugin-development/sdk/#stable-commerce-events)。
...
- `examples/plugins/hello-event`：设置、事件、翻译和
  schema 管理页。
- `examples/plugins/forum-extension`：论坛字段、
  颗粒化审核权限和定向 UI action slot。
- `examples/plugins/commerce-fulfillment`：
  `commerce.order.paid` after-commit 事件、幂等履约任务、严格的自定义 provider 合同
  和失败重试。

三者均可直接执行 validate、test、build、安装、启停和卸载生命周期。
