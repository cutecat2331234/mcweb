...
test('shared evidence uploader owns transport and scanning without domain copy', () => {
  ...
  assert.match(uploader, /body\.append\('subject_key', subjectKey\)/)
  ...
  assert.match(uploader, /onBeforeUnmount/)
  assert.match(uploader, /watch\(/)
  assert.match(uploader, /operationRevision/)
  assert.match(uploader, /activeController\?\.abort\(\)/)
  assert.match(uploader, /UPLOAD_REQUEST_TIMEOUT_MS = 60_000/)
  assert.match(uploader, /SCAN_REQUEST_TIMEOUT_MS = 10_000/)
  assert.match(uploader, /SCAN_DEADLINE_MS = 90_000/)
  assert.match(uploader, /fetchWithTimeout/)
  assert.match(uploader, /operationSignal\.aborted/)
  assert.match(uploader, /payload\.public_id !== initial\.public_id/)
  assert.match(uploader, /Number\.isFinite\(record\.byte_size\)/)
  assert.match(uploader, /attachmentPayload\(await responsePayload\(response\)\)/)
