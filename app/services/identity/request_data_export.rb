  ...
  class RequestDataExport < ApplicationService
    ...
    def call
      ...
      DataExport.transaction(requires_new: true) do
        ...
        data_export = DataExport.find_by(user: locked_user, idempotency_key: @idempotency_key)
        if data_export
          replayed = true
        elsif DataExport.where(user: locked_user, requested_at: 24.hours.ago..).count >= DAILY_LIMIT
          rate_limited = true
        else
      ...
      return failure("data_export_rate_limited") if rate_limited

      ServiceResult.success(data_export:, replayed:)
