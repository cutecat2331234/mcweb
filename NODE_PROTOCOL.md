...
控制面在任意时刻只允许一个父任务团占用全局派发槽。该任务团内部的不同物理节点可以并行处理各自批次，但即使某个节点提前空闲，也不能领取下一父任务团；只有当前父任务团的所有批次均收到最终确认并汇总为成功或错误，Sidekiq 才会释放派发槽并启动队首任务团。

同一个 Go 节点在任意时刻也只能有一个活动批次。状态和握手如下：
