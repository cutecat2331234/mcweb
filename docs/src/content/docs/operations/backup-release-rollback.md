---
title: 生产备份、恢复、更新与回滚
description: 创建并验证加密备份，执行恢复演练、安全更新和可审计回滚。
edition: ce
audience:
  - operator
---

适用于 CE 原生 systemd 与 release 目录部署。正式变更前必须在目标环境完成本文列出的
备份验证、恢复演练和回滚门禁；仓库测试不能代替生产环境演练。
...
并只允许 `mcweb_acceptance_*` 数据库。保存完整成功日志和生成的校验结果；静态合同或
使用临时数据库的测试不能替代这项演练。
