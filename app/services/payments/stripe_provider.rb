...
module Payments
  class StripeProvider < Provider
    ...
    def create_payment(payment_record, return_url_base: nil)
      ...
      return ServiceResult.failure(error: "Stripe is not enabled or fully configured.") unless config
      ...
      ServiceResult.failure(
        error: "Stripe could not create the payment session. Please try again.",
    ...
    def process_webhook_event(event)
      ...
      if payload_type.present? && !ActiveSupport::SecurityUtils.secure_compare(payload_type, event_type)
        return ServiceResult.failure(error: "Stripe event type mismatch.", code: "event_mismatch")
      end
    ...
    def process_refund(refund)
      ...
      unless refund.payment_record.provider_mode == config.effective_mode
        return ServiceResult.failure(
          error: "The payment Stripe environment is unknown or differs from the active provider.",
      ...
      unless payment_intent_id.match?(/\Api_[A-Za-z0-9_]+\z/)
        return ServiceResult.failure(
          error: "Stripe payment intent is unavailable for this payment.",
    ...
    def process_payment_success(event)
      ...
      return ServiceResult.failure(error: "Payment record not found.", code: "payment_not_found") unless payment_record
      ...
      if checkout_session_event?(event.event_type) && object["payment_status"] != "paid"
        ...
        return ServiceResult.failure(error: "Stripe checkout session is not paid.", code: "payment_not_paid")
      end
    ...
    def process_payment_failure(event)
      ...
      payment_record.with_lock do
        if payment_record.pending? || payment_record.processing?
          attributes = {
            metadata: payment_record.metadata.merge(
          ...
          attributes[:status] = "failed" unless RETRYABLE_PAYMENT_FAILURE_EVENTS.include?(event.event_type)
          payment_record.update!(attributes)
        end
      end
    ...
    def payment_event_error(payment_record:, object:, event_type:, succeeded:)
      return ServiceResult.failure(error: "Stripe event object is missing.", code: "object_missing") if object.empty?
      return ServiceResult.failure(error: "Payment provider mismatch.", code: "provider_mismatch") unless payment_record.provider == "stripe"

      config = Payments::ProviderConfig.checkout_config_for("stripe")
      return ServiceResult.failure(error: "Stripe configuration is unavailable.", code: "provider_unavailable") unless config

      if payment_record.provider_mode.blank? ||
          payment_record.provider_mode != config.effective_mode
        return ServiceResult.failure(
          error: "Stripe payment environment mismatch.",
          code: "environment_mismatch"
        )
      end
      ...
      unless object["livemode"] == expected_livemode
        return ServiceResult.failure(error: "Stripe environment mismatch.", code: "environment_mismatch")
      end
      ...
      unless metadata["payment_record_id"].to_s == payment_record.id.to_s &&
        ...
        return ServiceResult.failure(error: "Stripe payment metadata mismatch.", code: "metadata_mismatch")
      end
      ...
      if checkout_session_event?(event_type)
        unless object["id"].to_s == payment_record.provider_payment_id.to_s &&
          ...
          return ServiceResult.failure(error: "Stripe checkout session mismatch.", code: "session_mismatch")
        end
        ...
        if known_payment_intent.present? && object["id"].to_s != known_payment_intent
          return ServiceResult.failure(error: "Stripe payment intent mismatch.", code: "payment_intent_mismatch")
        end
      ...
      unless provider_amount.to_i == payment_record.amount_cents &&
        ...
        return ServiceResult.failure(error: "Stripe payment amount or currency mismatch.", code: "amount_mismatch")
      end
    ...
    def checkout_return_urls(order, supplied_base)
      ...
      unless %w[http https].include?(uri.scheme) && uri.host.present? && uri.userinfo.blank?
        return ServiceResult.failure(error: "The public checkout return URL is invalid.")
      end
      if Rails.env.production? && uri.scheme != "https"
        return ServiceResult.failure(error: "The public checkout return URL must use HTTPS in production.")
      end
      ...
      ServiceResult.failure(error: "The public checkout return URL is invalid.")
    end
    ...
    def refund_result(provider_refund, refund, payment_intent_id, expected_mode:)
      ...
      unless provider_refund_id.start_with?("re_") &&
        ...
        return ServiceResult.failure(
          error: "Stripe refund response does not match the local refund.",
      ...
      if provider_status == "succeeded"
        ...
        ServiceResult.failure(
          error: "Stripe refund is still pending.",
      ...
      record.with_lock do
        if record.provider_mode.blank?
          ...
          result = ServiceResult.failure(
            error: "The payment Stripe environment differs from the active provider.",
            code: "environment_mismatch"
          )
        end
      end
