...
McWeb 采用 **平台内核 + 大应用 + 插件扩展** 的分层设计，将官网、社区、商城与游戏内发货串联在同一套系统中，供服主自建部署与运营。
...
- **功能开关**：`Commerce::StoreFeatures` 控制实体商品、物流、礼品包装等子能力
...
McWeb 通过配置、协议与模板与外部系统衔接，主要方式如下：

| 方式 | 说明 |
|------|------|
| **FeatureFlags** | 论坛 / 商城 / Minecraft / 博客等大应用入口开关 |
| **Commerce::StoreFeatures** | 商城内物流、实体商品、礼品包装等子功能 |
| **ZIP 前台模板** | Portal / 商城皮肤：颜色 token、CSS、HTML 插槽 |
| **McWeb Connector** | 游戏服拉取任务、上报事件、账号绑定 |
| **mcweb-node** | 宿主机进程管理、备份、指标、Connector 本地代理 |
| **出站 Webhook** | 订单生命周期、论坛事件、保存搜索匹配推送至外部系统 |
...
完整权限列表见 [PERMISSIONS.md](PERMISSIONS.md)。
...
本仓库开源版本采用 **GNU Affero General Public License v3.0**（AGPL-3.0），全文见 [LICENSE](LICENSE)。

McWeb 另计划提供商业版发行渠道；商业许可条款预留于 [LICENSE-COMMERCIAL.md](LICENSE-COMMERCIAL.md)，与 AGPL 开源版并行存在。
