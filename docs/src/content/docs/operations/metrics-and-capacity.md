---
title: 运行指标与容量趋势
description: 查看隐私保护的运行指标、阈值、趋势、保留和清理状态。
edition: ce
audience:
  - operator
  - administrator
...
部署后在后台任务页确认指标刷新和每日清理任务均有近期成功记录，再依次切换 1 小时、
24 小时和 7 天范围。每个范围应显示当前与上一周期，并且不出现 URL、SQL bind、用户 ID、
邮箱或 token 等敏感维度。出现 `query_budget_exceeded` 时不要把不完整结果当作真实趋势；
先缩短范围并调查指标键数量与保留积压。
