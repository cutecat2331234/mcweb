...
module Admin
  module Forum
    class EventWebhookDeliveriesController < Admin::BaseController
      ...
      def index
        scope = Community::EventWebhookDelivery.includes(:topic, :post).recent
        ...
        render inertia: "Admin/Generic/Index", props: {
          ...
          columns: [
            admin_column(:event, t("mcweb.admin.forum.event_webhook_deliveries.col_event"), link: true),
            admin_column(:topic, t("mcweb.admin.forum.event_webhook_deliveries.col_topic")),
            admin_column(:status, t("mcweb.admin.forum.event_webhook_deliveries.col_status")),
            admin_column(:code, t("mcweb.admin.forum.event_webhook_deliveries.col_code")),
            admin_column(:attempts, t("mcweb.admin.forum.event_webhook_deliveries.col_attempts")),
            admin_column(:created_at, t("mcweb.admin.forum.event_webhook_deliveries.col_created_at"))
          ],
      ...
      def show
        render inertia: "Admin/Generic/Show", props: {
          title: t("mcweb.admin.forum.event_webhook_deliveries.show_title", id: @delivery.id),
          ...
          fields: [
            ...
            { label: t("mcweb.admin.forum.event_webhook_deliveries.field_time"), value: l(@delivery.created_at, format: :short) }
          ],
          preformattedSections: preformatted_sections,
          actions: show_actions,
          backUrl: admin_forum_event_webhook_deliveries_path(status: params[:status])
        }
      end
      ...
      def retry
        ...
        if result.success?
          redirect_to admin_forum_event_webhook_delivery_path(@delivery), notice: t("mcweb.flash.webhook_requeued")
        else
          redirect_to admin_forum_event_webhook_delivery_path(@delivery), alert: service_error_message(result)
        end
      end

      def bulk_retry
        result = Community::BulkRetryForumEventWebhooks.call(delivery_ids: params[:ids])
        if result.success?
          redirect_to admin_forum_event_webhook_deliveries_path(webhook_filter_params),
    ...
    private
      ...
      def set_delivery
        @delivery = Community::EventWebhookDelivery.find(params[:id])
      end

      def webhook_status_tabs
        base_params = webhook_filter_params.except(:status)
        base = admin_forum_event_webhook_deliveries_path(base_params)
        current = params[:status].to_s
        [
          { label: t("mcweb.admin.forum.filter_all"), href: base, active: current.blank? },
          { label: t("mcweb.admin.forum.webhook_deliveries.status_failed"), href: admin_forum_event_webhook_deliveries_path(base_params.merge(status: "failed")), active: current == "failed" },
          { label: t("mcweb.admin.forum.webhook_deliveries.status_success"), href: admin_forum_event_webhook_deliveries_path(base_params.merge(status: "success")), active: current == "success" },
          { label: t("mcweb.admin.forum.webhook_deliveries.status_pending"), href: admin_forum_event_webhook_deliveries_path(base_params.merge(status: "pending")), active: current == "pending" }
        ]
      end
      ...
      def webhook_event_tabs
        ...
        tabs = [ { label: t("mcweb.admin.forum.filter_all_events"), href: base, active: current.blank? } ]
        Community::DispatchForumEventWebhook::EVENT_TYPES.each do |event_type|
          tabs << {
            label: event_type,
            href: admin_forum_event_webhook_deliveries_path(base_params.merge(event: event_type)),
            active: current == event_type
          }
        end
        tabs
      end
      ...
      def webhook_kind_tabs
        ...
        [
          ...
          { label: t("mcweb.admin.forum.webhook_deliveries.kind_test"), href: admin_forum_event_webhook_deliveries_path(base_params.merge(kind: "test")), active: current == "test" },
          { label: t("mcweb.admin.forum.webhook_deliveries.kind_production"), href: admin_forum_event_webhook_deliveries_path(base_params.merge(kind: "production")), active: current == "production" }
        ]
      end
      ...
      def serialize_index_row(delivery)
        admin_row(
          ...
          url: admin_forum_event_webhook_delivery_path(delivery, status: params[:status], event: params[:event])
        )
      end
        ...
        [
          {
            ...
            href: retry_admin_forum_event_webhook_delivery_path(@delivery),
            method: "post"
          }
        ]
      end
